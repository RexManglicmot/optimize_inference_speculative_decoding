...
# -------------------------
_EMBED = None
def _ensure_embedder(device: torch.device):
    global _EMBED
    if _EMBED is None:
        try:
            from sentence_transformers import SentenceTransformer
            _EMBED = SentenceTransformer("sentence-transformers/all-MiniLM-L6-v2", device=str(device))
        except Exception as e:
            print(f"[audit] Semantic model unavailable ({e}). Similarity will be NaN.")
            _EMBED = None

def _cos_sim(a: str, b: str) -> float:
    if _EMBED is None:
        return float("nan")
    va = _EMBED.encode(a, normalize_embeddings=True)
    vb = _EMBED.encode(b, normalize_embeddings=True)
...
# -------------------------
def main():
    parser = argparse.ArgumentParser(description="Audit SD: acceptance, disagreement, semantic similarity")
    ...
    v_tok, v_model, v_id = _norm_verifier_tuple(verifier)
    v_model.eval().to(device)
    _set_pad_eos(v_tok, v_model)
    _ensure_embedder(device)

    spec_texts_path = "outputs/texts_specdec.jsonl"
    base_texts_path = "outputs/texts_baseline.jsonl"
    ...
    with tqdm(total=len(drafts), desc="Drafts", unit="draft", ncols=100) as outer_bar:
        for draft_entry in drafts:
            ...
            for i in tqdm(subset_idx, desc=f"{d_id}", leave=False, ncols=100):
                ...
                disagrees += int(spec_text.strip() != base_text.strip())
                sims.append(_cos_sim(spec_text, base_text))
            ...
            sim_arr = np.array(sims, dtype=float)
            rows.append({
                "draft_id": d_id,
                ...
                "disagreement_rate": float(disagrees / max(1, len(subset_idx))),
                "semantic_sim_mean": float(np.nanmean(sim_arr)),
            })
